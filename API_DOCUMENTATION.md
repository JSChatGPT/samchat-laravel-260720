...
- **E2EE key recovery.** Each device has its own identity keypair; the server only ever stores/relays opaque sealed copies of each chat's key (`chat_key_grants`), never anything it could decrypt itself. A reinstall/new phone generates a brand-new identity with no link to the old one, so by construction it can't read old messages until *something* reseals the chat's key to it. Two independent mechanisms now close that gap without any server-side key escrow: (1) self-heal — `ChatKeyController::missingDevices`/`requestGrant`, mirrored identically on mobile (`E2eeService.healMissingGrants`/`ensureChatKeyAvailable`) and web (`resources/js/chat.js`) — fires proactively on every chat-open and every inbox-load, not just on send, so recovery no longer needs both parties online at the same moment; and (2) an optional client-side encrypted backup (mobile only — `lib/core/crypto/backup/` in the Flutter app) of just the ~32-byte identity private key to the user's own Google Drive (`appDataFolder`) or iCloud (`NSUbiquitousKeyValueStore`), encrypted with a password only the user knows (Argon2id + the same ChaCha20-Poly1305 AEAD used for messages) — the server never sees it, and restoring it re-registers the *same* public key the backend already has grants for, recovering every chat's history immediately with no waiting. Residual limit, inherent to true E2EE and not fixable without weakening it: if literally every device that ever held a chat's key is gone forever with no backup ever enabled, that history is unrecoverable — but a normal user should no longer actually encounter this. The Android backup path needs a one-time Google Cloud Console setup (enable the Drive API, create an OAuth Android client with the app's SHA-1) that hasn't been done yet; the iOS path (`ios/Runner/ICloudBackupChannel.swift`) was written without any macOS/Xcode available and is unverified.
